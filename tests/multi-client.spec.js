...
// The four mock buzzers used throughout the test.
// Blade (BB0001) is always the active buzzer / winner.
const BUZZERS = [
  { id: 'BB0001', name: 'Blade'    },
  { id: 'BB0002', name: 'Player 2' },
  { id: 'BB0003', name: 'Player 3' },
  { id: 'BB0004', name: 'Noob'     },
];

/** Scroll into view then click — avoids fixed bottom dock intercepting. */
async function scrollClick(locator) {
...
test.describe('Multi-client Quiz Flow', () => {
  ...
  test.beforeAll(async () => {
    ...
    if (fs.existsSync('videos/multi-client.txt')) {
        try { fs.unlinkSync('videos/multi-client.txt'); } catch(e) {}
    }
    ...
    // Ensure the default question pack is active (XEON tests may have switched it)
    await fetch(`http://localhost:${testConfig.ports.ws}/api/packs/activate`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ questions: 'default' }),
    });
  ...
  test('Quiz flow with moderator and display', async ({ browser }) => {
    ...
    await test.step('Quiz Round: Blade Wins', async () => {
        ...
        const showAnswersBtn = moderatorPage.locator('.reveal-btns .btn.btn--accent');
        await expect(showAnswersBtn).toBeVisible({ timeout: 10000 });
        await scrollClick(showAnswersBtn);
        await moderatorPage.waitForTimeout(2000);

        // Verify buzzers are armed before pressing
        await expect(moderatorPage.locator('.question-answer').first()).toBeVisible({ timeout: 5000 });

        await buzzerList[0].press(); // Blade buzzes first
        ...
        await moderatorPage.waitForTimeout(2000);
        await scrollClick(moderatorPage.locator('.question-answer--clickable').nth(0));
        await moderatorPage.waitForTimeout(2000);
    ...
    await test.step('Quiz Round: Noob Loses', async () => {
        ...
        // Arm buzzers
        await scrollClick(moderatorPage.locator('.reveal-btns .btn.btn--accent'));
        await moderatorPage.waitForTimeout(1500);
        ...
        await scrollClick(moderatorPage.locator('.question-answer--clickable').nth(1));
        await moderatorPage.waitForTimeout(1500);
    ...
    fs.appendFileSync('videos/multi-client.txt', `${displayVideoPath}\n${moderatorVideoPath}\n${displayAudioPath || ''}\n`);

    // Attempt merge, then clean up raw files
    try {
        execSync('node tests/merge-videos.js videos/multi-client.txt', { stdio: 'inherit' });
